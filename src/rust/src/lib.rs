...
// Orthogonal projection of columnar vectors in mat onto v
...
fn gradient(mat: ArrayView2<f64>, v: ArrayView1<f64>, eps: f64) -> Array1<f64> {
    ...
    for i in 0..v.len() {
        ...
        grad[i] = ( orth_proj(mat,v_h.view()).var(1.0) - orth_proj(mat, v).var(1.0) ) / eps;
...
fn find_pd(mat: ArrayView2<f64>, eps: f64, tol: f64, max_iter: i32) -> Array1<f64> {
    ...
    let mut cur_v: Array1<f64> = Array1::random(nfeat.into_shape(), rand_distr::Uniform::new(-1.0, 1.0));
...
fn my_pca(mat: ArrayView2<f64>, k: i32, eps: f64, tol: f64, max_iter: i32) -> Robj {
    ...
    if k > mat.nrows() as i32 {
        throw_r_error("Desired number of principal components(k) greater than number of features present in data matrix");
    }
    ...
    for _ in 0..k {
        ...
        let pd = find_pd(mat.view(), eps, tol, max_iter);
        
        // Get principal component
        let o = orth_proj(mat.view(), pd.view());
